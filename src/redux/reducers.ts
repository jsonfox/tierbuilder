import {
  ADD_ROW, CHANGE_ROW_COLOR, CLEAR_ALL_ROWS,
  CLEAR_ROW, MOVE_ITEM, MOVE_ROW,
  REMOVE_ROW, RENAME_ROW, RESET, SET_DATA
} from './actions';
...
const actions = {
  ...
  MOVE_ITEM: (state: TbRow[], { dropInfo }: { dropInfo: DropResult }) => {
    ...
    let [fromRow, toRow] = [parseInt(source.droppableId), parseInt(destination!.droppableId)];
    if (toRow < fromRow) toRow++
    let [fromIndex, toIndex] = [source.index, destination!.index]
    if (toIndex < fromIndex) toIndex++
    const [fromItems, toItems] = [[...state[fromRow].items], [...state[toRow].items]]
    if (fromRow !== toRow) {
  ...
  MOVE_ROW: (state: TbRow[], { direction, rowIndex }: MoveRowAction) => {
    if ((rowIndex < 1) || (rowIndex < 2 && direction === 'up') || (rowIndex === state.length - 1 && direction === 'down')) return;
...
export const tierbuilder = (state: TbRow[] = initialState, action: any) => {
  ...
  switch (action.type) {
    ...
    case MOVE_ITEM: return actions[MOVE_ITEM](state, action)
    case ADD_ROW: return actions[ADD_ROW](state, action)
    case MOVE_ROW: return actions[MOVE_ROW](state, action)
    case REMOVE_ROW: return actions[REMOVE_ROW](state, action)
    case CLEAR_ROW: return actions[CLEAR_ROW](state, action)
    case CLEAR_ALL_ROWS:return actions[CLEAR_ALL_ROWS](state)
    case RENAME_ROW:return actions[RENAME_ROW](state, action)
    case CHANGE_ROW_COLOR: return actions[CHANGE_ROW_COLOR](state, action)
