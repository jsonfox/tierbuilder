...
export default function SettingsModal({ isOpen, color, name, rowIndex, setIsOpen, changeName }: Props) {
  ...
  const changeRowColor = (newColor: ColorResult) => dispatch({
      ...
      <div>
        ...
        <div className="modal-colorpicker">
          {showPicker ?
            (
              <>
                <button className="colorpicker-toggle" onClick={() => setShowPicker(false)}>
            ...
            : (
              <>
                <button className="colorpicker-toggle" onClick={() => setShowPicker(true)}>
                  Choose custom color
                </button>
                <CirclePicker
                  color={color}
